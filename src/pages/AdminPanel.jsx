...
import authors_action from '../store/actions/authors&companies.js'
import moment from 'moment/moment.js';

const { get_authors } = authors_action
const { get_companies } = authors_action
...
export default function AdminPanel(){
    ...
    let getData = useSelector(store => store.getData);
    console.log(getData)
    let getAuthors = getData.getAuthors
    let getCompanies = getData.getCompanies
                                                ...
                                                <div key={company.name} className="h-auto w-full flex flex-wrap justify-center items-center sm:flex sm:justify-between sm:items-center p-2 border-gray-200 border-b-2">
                                                    <table className="w-full flex flex-row justify-center">
                                                        <tbody className="w-full">
                                                            <tr className="flex flex-wrap gap-3 w-full justify-evenly sm:w-full sm:flex sm:flex-row sm:justify-between text-[#484964] font-medium">
                                                                ...
                                                                <td>
                                                                    <label className="relative inline-flex items-center cursor-pointer">
                                                                        <AdminSwitch company={company} url={"http://localhost:8000/auth/role/company/"} params={company._id} />
                                                                    </label>
                                                                </td>
                                                            </tr>
                                                        </tbody>
                                        ...
                                        <div className="flex flex-col h-auto w-full">
                                            {getAuthors.map((author) => (
                                                <div key={author.name} className="h-auto w-full flex flex-wrap justify-center items-center sm:flex sm:justify-between sm:items-center p-2 border-gray-200 border-b-2">
                                                    <table className="w-full flex flex-row justify-center">
                                                        <tbody className="w-full">
                                                            <tr className="flex flex-wrap gap-3 w-full justify-center sm:w-full sm:flex sm:flex-row sm:justify-between text-[#484964] font-medium">
                                                                <td className="flex gap-x-1">
                                                                ...
                                                                <td>
                                                                    <label className="relative inline-flex items-center cursor-pointer">
                                                                        <AdminSwitch author={author} url={"http://localhost:8000/auth/role/author/"} params={author._id} />
                                                                    </label>
                                                                </td>
                                                            </tr>
                                                        </tbody>
